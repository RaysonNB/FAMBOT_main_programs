...
class FollowMe(object):
    ...
    def find_cx_cy(self) -> Tuple[int, int]:
        max_=1
        global dnn_follow, rgb_image, pree_cx,pree_cy
        cx,cy=0,0
        rcx,rcy=0,0
...
if __name__ == "__main__":    
    ...
    while not rospy.is_shutdown():
            ...
            if pose is not None:
                ...
                if len(detections)==1 or step=="get1":
                    ...
                    if bx<0:
                        ...
                        rcnt+=1
                print(rcnt, lcnt, one)
                if haiya ==1 or (run == 1) or haiya == 2 or step2=="go" or step2=="goo":
                    if cx !=0:
                        say("turn")
                        time.sleep(1)
                        run=0
                        ...
                        cv2.imwrite(output_dir + filename, rgb_image)
                        time.sleep(4)
                        if haiya==1 and step=="goo":
                        ...
                        if step=="go1":
                            ...
                            for i in range(cy + 1, h):
                                if _depth[cy][cx] == 0 or 0 < _depth[i][cx] < _depth[cy][cx]:
                                    cy = i 
                            _,_,d = get_real_xyz(_depth,cx,cy)
                            while d > 0 or abs(e) >= 1:
                                _,_,d1 = get_real_xyz(_depth,cx,cy)
                                e = d1 - 150 #number is he last distance
                                if e<=1:
                                    break
                                v = 0.001 * e
                                if v > 0:
        ...
        if step == "follow":
            ...
            x, z, rgb_image = _fw.calc_cmd_vel(rgb_image, depth)
            msg.linear.x = x 
            msg.angular.z = z
            _cmd_vel.publish(msg)
            s=s.lower()
        ...
        if step == "back":
            ...
            time.sleep(1)
            '''
            chassis.move_to(2.88,1.67,0.169)
            #checking
            while not rospy.is_shutdown():
                # 4. Get the chassis status.
                code = chassis.status_code
